...
def assert_auth_config() -> None:
    ...
    if not auth_password():
        raise RuntimeError(
            "APP_PASSWORD is required when running on ECS Express or APP_ENV=production."
        )
    if not auth_secret_key():
        raise RuntimeError(
            "APP_SECRET_KEY is required when running on ECS Express or APP_ENV=production."
        )
